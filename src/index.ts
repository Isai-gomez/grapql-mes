...
export default class SnapshotTransform implements MeshTransform {
  ...
  constructor({
    ...
    config,
    importFn,
  }: MeshTransformOptions<YamlConfig.SnapshotTransformConfig>) {
    this.config = config;
  ...
  transformSchema(schema: GraphQLSchema) {
    ...
    if (configIf) {
      ...
      const snapshotComposition: ResolversComposition =
        (next) => async (root, args, context, info) => {
          ...
          if (
            snapshotFilePath in require.cache ||
            (await pathExists(snapshotFilePath))
          ) {
